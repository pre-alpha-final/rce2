...
- **Auth (optional)**: per-agent key sent as an HTTP Basic `Authorization` header containing the base64-encoded raw key. Broker-side enforcement is gated by `ForceAgentAuth` in `appsettings.json`. The broker UI/management API (`/api/broker`) is `[Authorize]` and uses `BrokerKey` from `appsettings.json`.
